...
class OpenPMDGrid(AMRGridPatch):
    ...
    def __init__(
        self, gid, index, level=-1, fi=0, fo=0, pi=0, po=None, ft=None, pt=None
    ):
    ...
    def _read_particles(self):
        ...
        # quick check
        print(
            len(self.index.grids),
            len(
                f.particles[list(f.particles)[0]]["position"][
                    axes_labels[0]
                ].available_chunks()
            ),
        )

        assert len(self.index.grids) == len(
            f.particles[list(f.particles)[0]]["position"][
                axes_labels[0]
            ].available_chunks()
        )
    ...
    def NumParticles(self):
        ...
        if (
            "particlesPatches"
            in self.index.dataset._handle.particles[
                list(self.index.dataset._handle.particles)[0]
            ]
        ):
            for species in self.index.dataset._handle.particles:
                for _patch in list(
                    self.index.dataset._handle.particles[str(species)][
                        "particlePatches"
                    ]
                ):
                    # TODO Add support for ParticlPatches
                    mylog.warning(
                        "ParticlePatches is not yet supported as it has not yet been tested :("
                    )
            print("PartPatches")
        else:
...
class OpenPMDHierarchy(GridIndex):
        ...
        try:
            ...
            for level, mname in enumerate(list(f.meshes)[: self.max_level + 1]):
                ...
                if isinstance(mesh, openpmd_api.io.openpmd_api_cxx.Mesh):
                    ...
                    )  # per-level domain shape, not grid shape
                else:
                    raise AttributeError
    ...
    def _parse_index(self):
        ...
        grid_index_total = 0
        self.level_dds = np.empty(
            (self.max_level + 1, 3)
        )  # we always pad to three dimensions regardless of coordinate system
        # Mesh grids
    ...
    def _parse_parameter_file(self):
        ...
        try:
            ...
            for mname, mesh in f.meshes.items():
                ...
                left_edges[mname] = (
                    le  # component_ordering(le, self._geometry, self._data_order, self._axes_labels)
                )
                right_edges[mname] = (
                    re  # component_ordering(re, self._geometry, self._data_order, self._axes_labels)
                )
