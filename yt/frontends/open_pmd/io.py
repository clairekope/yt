...
class IOHandlerOpenPMD(BaseIOHandler):
    ...
    def _fill_cache(self, ptype, index=0, offset=None):
        ...
        if str((ptype, index, offset)) not in self._cached_ptype:
            ...
            for _ax in {"x", "y", "z"} - set(self.ds._axes_labels):
                # Pad accordingly with 0.5 to make 1D/2D datasets compatible
                # as missing dimensions are given empty bounds [0,1]
                # These have to be the same shape as the existing axes since that
                # equals the number of particles. Only work for Cartesian coordinates
                i += 1
                self.cache[i] = np.full(offset, 0.5, dtype=np.float64)

    # TODO should we have these functions as suggested by the creating a new code frontend page?
    ...
    def _read_particle_selection(self, chunks, selector, fields):
        ...
        for ptype, pname in fields:
            ...
            ind[pfield] = 0
            print(
                ptype, pname, ptf[ptype], rfm[pfield], "HERENOW", particle_count[pfield]
            )
            ...
            for chunk in chunks:
                ...
                for grid in chunk.objs:
                    ...
                    print(
                        ...
                        len(
                            ds[species]["position"][
                                list(ds[species]["position"])[0]
                            ].available_chunks()
                        ),
                    )
                    grid.pindex = (
                        ds[species]["position"][list(ds[species]["position"])[0]]
                        .available_chunks()[grid.id]
                        .offset[0]
                    )
                    grid.poffset = (
                        ds[species]["position"][list(ds[species]["position"])[0]]
                    ...
                    for field in ptf[ptype]:
                        ...
                        if len(record) > 1:  # specifying axes here
                            ...
                            if (
                                record == "position"
                                and is_const_component(pds[record][component])
                            ...
                            data = get_component(
                                ...
                                list(pds[component])[
                                    0
                                ],  # takes care of the SCALAR key for openpmd_api
                                grid.pindex,
                                grid.poffset,
                            )[mask]  # [grid._pdata[str(species)]['mask']]
                        ...
                        for request_field in rfm[ptype, field]:
                            print(
                                "shapes", ind[request_field], data.shape, np.shape(data)
                            )
        ...
        for ftype, fname in fields:
            ...
            for chunk in chunks:  # one chunk per level?
                for grid in chunk.objs:  # grids per level
                    ...
                    # print(count,'count')
                    rv[field][ind[field] : ind[field] + count] = data[
                        mask
                    ]  # multiply by unit SI here?
